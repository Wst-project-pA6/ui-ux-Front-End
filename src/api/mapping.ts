...
export function errorMessage(code: string, fallback: string): string {
  switch (code) {
    ...
    case 'ASSESSMENT_LOCKED': return 'This assessment is signed off and can no longer be changed.'
    case 'CERTIFICATE_NOT_ELIGIBLE': return 'Certificate cannot be issued yet — requirements are not fully met.'
    case 'CONFLICT_NOT_OVERRIDABLE': return 'This conflict cannot be overridden.'
    case 'EXPORT_NOT_READY': return 'The export is not ready yet — please wait and retry.'
    case 'EXPORT_EXPIRED': return 'This export has expired — please generate a new one.'
