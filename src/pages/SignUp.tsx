...
export default function SignUp() {
  ...
  const validatePassword = (v: string) => {
    ...
    if (!isStrongEnough(v)) return t('auth.error.weakPassword')
    return ''
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Contract truth: there is NO public sign-up endpoint. Users are created
    // by an admin via POST /users (permission users.manage). This demo form
    // only writes to the offline demo directory and is disabled against live API.
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-8 bg-slate-50 overflow-y-auto">
        <div className="w-full max-w-sm py-8">
          ...
          {/* Contract notice: no public registration endpoint */}
          <div role="note" className="mb-4 px-4 py-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-xs leading-relaxed">
            Demo only — the API contract has no public sign-up. Production accounts are created by an admin via POST /users.
          </div>
