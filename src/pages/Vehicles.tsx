...
export default function Vehicles() {
  ...
  const [customerOptions, setCustomerOptions] = useState<Array<{ id: string; label: string }> | null>(null)

  // Live customers (backend customerId must be a UUID). Demo fallback offline.
  React.useEffect(() => {
    if (!addOpen || customerOptions !== null) return
    let cancelled = false
    customersApi.list({ pageSize: 100, sort: 'displayName' })
      .then((res) => {
        if (!cancelled) setCustomerOptions(res.items.map((c) => ({ id: c.id, label: c.displayName })))
      })
      .catch(() => { if (!cancelled) setCustomerOptions(null) })
    return () => { cancelled = true }
  }, [addOpen, customerOptions])
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const mileageUnit = String(fd.get('mileageUnit') ?? 'KM') === 'MI' ? 'MI' : 'KM'
    const customerId = String(fd.get('customer') ?? '')
    if (!vin || !/^[A-HJ-NPR-Z0-9]{17}$/.test(vin)) {
      setSaveError('VIN is required and must be 17 characters (A-Z, 0-9, excluding I, O, Q).')
      return
    }
    if (!customerId) {
      setSaveError('Customer is required — select a live customer record.')
      return
    }
    setSaving(true)
    setSaveError('')
    try {
      // Backend: POST /vehicles (plate/VIN unique among active -> 409).
      await vehiclesApi.create({ customerId, plate, vin, make, model, year, mileage, mileageUnit })
      setAddOpen(false)
    } catch (err) {
    ...
    <div className="space-y-6">
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <Button onClick={() => { setSelected(null); navigate('/job-cards') }}>{t('vehicles.detail.newJobCard')}</Button>
            </>
          }
        >
      ...
      >
        <form id="vehicle-register-form" onSubmit={handleRegister} className="flex flex-col gap-4">
          ...
          <Input name="vin" label={t('vehicles.form.vin')} placeholder="Enter 17-character VIN" required />
          <div className="grid grid-cols-2 gap-4">
            <Input name="mileage" label={t('vehicles.form.mileage')} type="number" placeholder="0" />
            <Select name="mileageUnit" label="Mileage unit" options={[{ value: 'KM', label: 'KM' }, { value: 'MI', label: 'MI' }]} defaultValue="KM" />
          </div>
          <Select name="customer" label={t('vehicles.form.customer')} options={[
            { value: '', label: t('vehicles.form.selectCustomer') },
            ...(customerOptions ?? [
              { id: 'c1', label: 'Mohammed Al-Rashid' },
              { id: 'c2', label: 'Sarah Al-Anazi' },
              { id: 'c3', label: 'Rayan Omar' },
