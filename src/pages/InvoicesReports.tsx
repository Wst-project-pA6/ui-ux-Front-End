...
export default function InvoicesReports() {
  ...
  // Contract: POST /exports {exportType: INVOICES} (202) -> poll ->
  // download authorization. Offline: client-side CSV of visible rows.
  const handleExport = async () => {
    setExporting(true)
    setExportError('')
    try {
      const job = await exportsApi.create({ exportType: 'INVOICES', format: 'CSV' })
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {exporting ? '…' : t('invoices.exportCsv')}
            </button>
          </div>
        }
      />
