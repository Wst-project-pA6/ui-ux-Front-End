...
export default function InvoicesReports() {
  ...
  const handleExportCsv = () => {
    ...
    showToast('success', 'Export complete', `invoices.csv (${filtered.length} rows)`)
  }

  const handleExportPdf = () => {
    showToast('info', 'Export statement', 'Choose "Save as PDF" in the print dialog to download the statement.')
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle="Read-only financial overview"
        actions={
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Collected (Paid)</p>
          <p className="text-2xl font-bold text-green-600">{fmt(totalPaid)}</p>
          <p className="text-xs text-slate-400 mt-0.5">{invoices.filter((i) => i.status === 'paid').length} invoices</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Outstanding (Pending)</p>
          <p className="text-2xl font-bold text-amber-600">{fmt(totalPending)}</p>
          <p className="text-xs text-slate-400 mt-0.5">{invoices.filter((i) => i.status === 'pending').length} invoices</p>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-4">
          ...
          <p className="text-xs text-slate-400 mt-0.5">{invoices.filter((i) => i.status === 'overdue').length} invoices</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 border border-blue-100 rounded-lg text-sm text-blue-700">
        ...
        You have view and export access only. Invoice creation and editing requires a Workshop Manager or Service Advisor role.
      </div>
        ...
        <div className="flex flex-col gap-2 px-4 py-3 border-b border-slate-100 md:flex-row md:items-center md:gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by invoice, customer, or job card…"
            />
          </div>
          ...
          <span className="text-xs text-slate-400 md:ms-auto">{filtered.length} records</span>
        </div>
        ...
        <div className="md:hidden divide-y divide-slate-50">
          {filtered.map((inv) => (
            <div key={inv.id} className="px-4 py-3">
              ...
              {inv.paymentRef && (
                <p className="text-xs text-slate-400 font-mono mt-0.5" dir="ltr">Ref: {inv.paymentRef}</p>
              )}
            </div>
          ))}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-50">
                {['Invoice', 'Job Card', 'Customer', 'Date', 'Due Date', 'Amount', 'VAT (15%)', 'Total', 'Status', 'Ref'].map((h) => (
                  <th key={h} className="px-4 py-3 text-start text-xs font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((inv) => (
                <tr key={inv.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-500" dir="ltr">{inv.jobCard}</td>
                  ...
                  <td className="px-4 py-3 font-mono text-xs text-slate-400" dir="ltr">{inv.paymentRef ?? '—'}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
