...
import { laborApi, inventoryApi, jobsApi } from '../api/resources'
...
type JobStatus = 'received' | 'in-progress' | 'quality-check' | 'ready'
...
export default function MyJobs() {
  ...
  // Live parts catalog for the issue-part select (UUIDs required by the
  // backend). Falls back to demo entries when offline.
  React.useEffect(() => {
    if (!addPart || partOptions !== null) return
    let cancelled = false
    inventoryApi.parts({ pageSize: 100, sort: 'sku' })
      ...
      .catch(() => {
        if (!cancelled) setPartOptions(null)
      })
    return () => { cancelled = true }
  }, [addPart, partOptions])

  // Live stores for the same reason (backend storeId must be a UUID).
  React.useEffect(() => {
    if (!addPart || storeOptions !== null) return
    let cancelled = false
    inventoryApi.stores({ pageSize: 100 })
      ...
      .catch(() => {
        if (!cancelled) setStoreOptions(null)
      })
    return () => { cancelled = true }
  }, [addPart, storeOptions])

  const myJobs = jobs

  // Stage transition (same contract rule as JobCards: POST .../transitions).
  const handleTransition = async (toStage: 'IN_PROGRESS' | 'QUALITY_CHECK' | 'READY') => {
    if (!selected) return
    if (!isUuid(selected.id) && mode !== 'demo') {
      setSaveError('This preview job has no backend record to transition.')
      return
    }
    const fromStage = selected.status === 'received' ? 'RECEIVED' : selected.status === 'in-progress' ? 'IN_PROGRESS' : 'QUALITY_CHECK'
    setTransitioning(true)
    setSaveError('')
    try {
      if (isUuid(selected.id)) {
        await jobsApi.transition(selected.id, { toStage, expectedFromStage: fromStage as 'RECEIVED' | 'IN_PROGRESS' | 'QUALITY_CHECK' })
      }
      const next: JobStatus = toStage === 'IN_PROGRESS' ? 'in-progress' : toStage === 'QUALITY_CHECK' ? 'quality-check' : 'ready'
      applyToJob(selected.id, (j) => ({ ...j, status: next }))
    } catch (err) {
      if (err instanceof ApiError && err.status === 0) {
        // Offline demo: advance locally, clearly labeled.
        const next: JobStatus = toStage === 'IN_PROGRESS' ? 'in-progress' : toStage === 'QUALITY_CHECK' ? 'quality-check' : 'ready'
        applyToJob(selected.id, (j) => ({ ...j, status: next }))
      } else {
        const code = err instanceof ApiError ? err.code : ''
        setSaveError(errorMessage(code, err instanceof Error ? err.message : 'Transition failed'))
      }
    } finally {
  ...
  const handleSaveLabor = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const workDate = String(fd.get('workDate') ?? '').trim() || new Date().toISOString().slice(0, 10)
    ...
    if (!isUuid(selected.id) && mode !== 'demo') {
      setSaveError('This preview job has no backend record for labor entries.')
      return
    }
    if (!isUuid(selected.id)) {
    ...
    setSaveError('')
    try {
      // Backend LaborEntryCreateRequest: { workDate: YYYY-MM-DD,
      // durationMinutes: int, description? }. No rate field — the rate is
      // snapshotted server-side from finance settings.
  ...
  const handleIssuePart = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const partLabel = partOptions?.find((p) => p.id === partId)?.label ?? partId
    const storeId = String(fd.get('storeId') ?? '').trim()
    const quantity = Number(fd.get('quantity') ?? 1) || 1
    if (!partId || !storeId) {
      setSaveError('Part and store are required')
      return
    }
    if (!isUuid(selected.id) && mode !== 'demo') {
      setSaveError('This preview job has no backend record for part issues.')
      return
    }
    if (!isUuid(selected.id)) {
      applyToJob(selected.id, (j) => ({ ...j, partsIssued: [...j.partsIssued, `${partLabel} ×${quantity}`] }))
      setAddPart(false)
      return
    }
    setSaving(true)
    setSaveError('')
    try {
      // Contract PartIssueCreateRequest: { partId, storeId, quantity }.
      // Insufficient stock -> 409 INSUFFICIENT_STOCK (never negative).
          ...
          footer={
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {selected.status === 'in-progress' && (
                <Button loading={transitioning} onClick={() => handleTransition('QUALITY_CHECK')}>{t('jobCards.action.sendQC')}</Button>
              )}
              {selected.status === 'quality-check' && (
                <Button loading={transitioning} onClick={() => handleTransition('READY')}>{t('jobCards.action.markReady')}</Button>
              )}
              {selected.status === 'received' && (
                <Button loading={transitioning} onClick={() => handleTransition('IN_PROGRESS')}>{t('jobCards.action.startWork')}</Button>
              )}
            </div>
          }
        >
      ...
      >
        <form id="labor-add-form" onSubmit={handleSaveLabor} className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1">{t('myJobs.form.workDate')}</label>
              <input name="workDate" type="date" className="w-full h-10 px-3 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
          </div>
          <div>
      ...
      >
        <form id="part-issue-form" onSubmit={handleIssuePart} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <select name="partId" required className="w-full h-10 px-3 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">—</option>
              {(partOptions ?? [
                { id: 'Engine Oil 5W-30 (4L)', label: 'Engine Oil 5W-30 (4L) — 42 in stock' },
                { id: 'Oil Filter', label: 'Oil Filter — 18 in stock' },
                { id: 'Brake Pad Set (Front)', label: 'Brake Pad Set (Front) — 3 in stock' },
                { id: 'Air Filter', label: 'Air Filter — 11 in stock' },
              ]).map((p) => (
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1">{t('inventory.form.store')}</label>
            <select name="storeId" required className="w-full h-10 px-3 border border-slate-200 rounded-lg text-sm text-slate-700 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">—</option>
              {(storeOptions ?? [
                { id: 'Main Store', label: 'Main Store' },
                { id: 'Bay Store A', label: 'Bay Store A' },
              ]).map((s) => (
