...
import { trainingApi } from '../api/resources'
...
export default function Training() {
  ...
  const { mode } = useAuth()
  const { role } = useRole()
  // Students browse courses/sessions read-only; scheduling, publishing and
  // editing stay with the Training Supervisor.
  ...
  // Publish (WST-FR-10): POST .../transitions {toStatus: PUBLISHED}.
  // A conflicting session cannot be published — enforced, not just hidden.
  const handlePublish = async (s: DemoSession) => {
    ...
    try {
      ...
      setSessions((prev) => prev.map((x) => (x.id === s.id ? { ...x, status: 'ready' } : x)))
    } catch (err) {
  ...
  const handleEdit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const bayLabel = { b1: 'Bay 1', b2: 'Bay 2', b3: 'Bay 3', b4: 'Bay 4' }[bayId] ?? editing.bay
    ...
    try {
      ...
      setSessions((prev) => prev.map((x) => (x.id === editing.id
        ? { ...x, bay: bayLabel, mentor: mentor?.name ?? x.mentor, capacity }
        : x)))
      setEditing(null)
    } catch (err) {
      if (err instanceof ApiError && err.status === 0) {
  ...
  const handleSchedule = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!courseId || !mentorId || !bayId || !date || !startTime || !durationHrs) {
      setSaveError('Course, mentor, bay, date, start time and duration are required')
      return
    }
    const course = courses.find((c) => c.id === courseId)
    const mentor = mentors.find((m) => m.id === mentorId)
    const bayLabel = { b1: 'Bay 1', b2: 'Bay 2', b3: 'Bay 3', b4: 'Bay 4' }[bayId] ?? bayId
    ...
    try {
      ...
      await trainingApi.createSession({
        title: course?.title ?? 'Training session', courseId, groupId: 'G-2024-A',
      ...
      if (err instanceof ApiError && err.status === 0) {
        ...
        setSessions((prev) => [...prev, {
          id: `SES-LOCAL-${Date.now()}`, course: course?.title ?? courseId,
          mentor: mentor?.name ?? mentorId, bay: bayLabel, date, time: startTime,
    ...
    <div className="space-y-6">
            ...
            >
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                  {!session.conflict && session.status !== 'ready' && canManage && (
      ...
      >
        <form id="session-schedule-form" onSubmit={handleSchedule} className="flex flex-col gap-4">
          ...
          <Select name="course" label={t('training.form.course')} options={[{ value: '', label: t('training.form.selectCourse') }, ...courses.map((c) => ({ value: c.id, label: c.title }))]} required />
          <Select name="mentor" label={t('training.form.mentor')} options={[{ value: '', label: t('training.form.selectMentor') }, ...mentors.map((m) => ({ value: m.id, label: m.name }))]} required />
          <Select name="bay" label={t('training.form.bay')} options={[{ value: '', label: t('training.form.selectBay') }, { value: 'b1', label: 'Bay 1' }, { value: 'b2', label: 'Bay 2' }, { value: 'b3', label: 'Bay 3' }, { value: 'b4', label: 'Bay 4' }]} required />
      ...
      {editing && (
        ...
        >
          <form id="session-edit-form" onSubmit={handleEdit} className="flex flex-col gap-4">
            ...
            <Select name="mentor" label={t('training.form.mentor')} defaultValue="" options={[{ value: '', label: t('training.form.selectMentor') }, ...mentors.map((m) => ({ value: m.id, label: m.name }))]} />
            <Select name="bay" label={t('training.form.bay')} defaultValue="" options={[{ value: '', label: t('training.form.selectBay') }, { value: 'b1', label: 'Bay 1' }, { value: 'b2', label: 'Bay 2' }, { value: 'b3', label: 'Bay 3' }, { value: 'b4', label: 'Bay 4' }]} />
