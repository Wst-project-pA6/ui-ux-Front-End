...
export default function MyTraining() {
  ...
  const downloadCertificate = () => {
    // Certificates are issued by the supervisor; the student downloads the
    // issued record as PDF via the print dialog.
    showToast('info', 'Download certificate', 'Choose "Save as PDF" in the print dialog.')
          ...
          {sessions.map((session) => (
            <button
              ...
              className={`w-full text-start bg-white border border-slate-200 rounded-xl p-4 transition-all ${
                session.tasks.length > 0 ? 'hover:border-blue-300 hover:shadow-sm group cursor-pointer' : 'cursor-default'
              }`}
            >
          ...
          >
            ...
            <div className="p-5 flex flex-col gap-4">
              ...
              <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${
                selected.signed ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'
              }`}>
                {selected.signed ? (
                  <>
