...
export default function Dashboard() {
  ...
  const handleExport = async () => {
    // Contract: POST /exports (202) -> poll GET /exports/{id} -> POST .../download-authorizations.
    // Offline/demo: fall back to the previous client-side CSV so the button never breaks.
    setExporting(true)
    try {
      const job = await exportsApi.create({ exportType: 'WORKSHOP_JOBS', format: 'CSV' })
      let status = job.status
      for (let i = 0; i < 10 && status !== 'COMPLETED' && status !== 'FAILED'; i++) {
      ...
      if (!(e instanceof ApiError && e.status !== 0)) {
        // Demo fallback: client-side CSV of the visible rows.
