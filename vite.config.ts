...
react(),
    ...
    server: {
      ...
      strictPort: true,
      // The NestJS backend (http://localhost:3000) serves the API at /api/v1
      // without CORS headers, so the dev server proxies /api to it — the same
      // integration the backend's own frontend uses (see its vite.config.ts).
